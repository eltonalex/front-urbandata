...
gulp.task('styles', function() {
	...
	return stream.done()
					...
					.pipe(gulp.dest('vendor/styles/'))
					.pipe(minify({keepSpecialComments: 0}))
					.pipe(rename({suffix: '.min'}))
					.pipe(plumber.stop()).pipe(gulp.dest('vendor/styles/')).pipe(browserSync.reload({stream: true}));
});
...
gulp.task('scripts', function() {
	...
	return stream.done()
					...
					.pipe(gulp.dest('vendor/scripts/')).pipe(rename({suffix: '.min'}))
					.pipe(uglify())
					.pipe(plumber.stop())
...
gulp.task('connect-sync', ['styles', 'scripts', 'php'], function() {
	...
	return gulp.watch(['*', 'vendor/**/**'], function(file) {
